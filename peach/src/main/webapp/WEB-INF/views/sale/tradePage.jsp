	...
	<c:choose>
		<c:when test="${empty trList}">
			<div class="message-container">
				...
				<img class="no-items-img" width="100px" height="100px"
					src="<c:url value='/img/RESISTX.gif'/>">
				...
				<div class="container1">
					<div class="kind">
						<div class="kind_slider">
							<ul class="slides">
								<c:forEach items="${tr.saleBoardVO.saleImageVOList}"
									var="saleImage">
									<li><img class="item"
										src="<c:url value='/resources/image/${tr.saleBoardVO.saleImageVOList.size() != 0 ? tr.saleBoardVO.saleImageVOList.get(0).si_name :\"\" }'/>"></li>
								</c:forEach>
							</ul>
						</div>
						...
						<div class="profile-right">
							<div class="profile-right-box">
								<span class="profile-sweetness-text">당도</span> <span
									class="profile-sweetness">${tr.saleBoardVO.sb_me_sugar}</span>
							</div>
						</div>
					</div>
					<div class="content-box">
						...
						<p class="category-date">${tr.saleCategoryVO.sc_name}|
							${tr.saleBoardVO.sb_date}</p>
					...
					<div class="button-box">
						<c:choose>
							<c:when
								test="${user.me_num == tr.tq_me_num && tr.tradingVO.tr_cu == 1}">
								<button type="button"
									onClick="location.href='<c:url value='/chat/chat?sb_num=${tr.saleBoardVO.sb_num}'/>'"
									class="chat">피치톡</button>
								<button id="underTake" type="button" class="pay"
									data-tq-num="${tr.tq_num}">인수완료</button>
								<button type="button" class="report-post"
									id="openReportModalBtn">거래취소 요청</button>
							</c:when>
							<c:otherwise>
								<c:choose>
									<c:when
										test="${user.me_num == tr.saleBoardVO.sb_me_num && tr.tradingVO.tr_se == 1}">
										<button type="button"
											onClick="location.href='<c:url value='/chat/chat?sb_num=0'/>'"
											class="chat">피치톡</button>
										<button id="giveItem" type="button" class="pay"
											data-tq-num="${tr.tq_num}">인계완료</button>
										<button type="button" class="report-post"
											id="openReportModalBtn">거래취소 요청</button>
									</c:when>
									<c:otherwise>
										<c:choose>
											<c:when
												test="${tr.tradingVO.tr_cu == 2 && tr.tradingVO.tr_se == 2}">
												<p>거래가 완료되었습니다.</p>
											</c:when>
											<c:otherwise>
												<c:choose>
													<c:when
														test="${user.me_num == tr.tq_me_num && tr.tradingVO.tr_cu == 2}">
			...
			<div class="modal-body">
				...
				<div class="report-text-area">
					...
					<textarea id="reportReason" class="report-reason"
						placeholder="거래취소 요청 사유를 입력하세요" maxlength="500"></textarea>
				</div>
			</div>
			<div class="modal-footer">
				<button class="report-button" onclick="reportPost()">거래취소
					요청</button>
