...
<body>
	...
	<div class="co_de CDdiv">
		...
		<div class="co_de_info CDdiv">
			<a href="<c:url value='/board//profile/+${writer.me_num}'/>">작성자:
				${writer.me_nick}</a> | 작성일: ${detail.co_date}
		</div>

		<c:if test="${user.me_num == writer.me_num}">
			<form action="<c:url value='/board/communityEdit/${detail.co_num}'/>"
				method="get">
									...
									<div class="profile-right-box">
										<c:if test="${user != null}">
											<button type="button" class="report-post"
												id="openReportModalBtn2" data-num="${reList.re_num}">신고</button>
										</c:if>
									</div>
								</div>
							</c:if>
	...
	<div id="reportPostModal2" class="custom-modal2">
		<div class="modal-content">
			...
			<div class="modal-body">
				...
				<div class="report-text-area">
					<p class="maxtext">* 최대 500자 제한</p>
					<textarea id="reportReason2" class="report-reason"
						placeholder="신고 이유를 입력하세요" maxlength="500"></textarea>
				</div>
			</div>
			<div class="modal-footer">
